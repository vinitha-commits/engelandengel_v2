...
export default function ExpertiseGridV3() {
  return (
          ...
          {expertiseAreas.map((area, index) => (
            ...
            >
              <Link href={area.href} className="group block">
                <div className="relative bg-primary-950/90 backdrop-blur-sm border border-white/10 rounded-lg p-6 text-left h-40 flex flex-col justify-between overflow-hidden hover:border-[#D4AF37]/40 hover:shadow-[0_0_30px_rgba(212,175,55,0.1)] transition-all duration-500">

                  {/* Background number */}
                  <span className="absolute -right-2 -bottom-4 text-[80px] font-bold text-white/[0.03] leading-none select-none">
                    {String(index + 1).padStart(2, '0')}
                  </span>

                  {/* Top row: icon + arrow */}
                  <div className="flex items-start justify-between relative z-10">
                    <svg className="w-6 h-6 text-[#D4AF37]" fill="none" stroke="currentColor" viewBox="0 0 24 24" strokeWidth={1.2}>
                      <path strokeLinecap="round" strokeLinejoin="round" d={area.icon} />
                    </svg>
                    <svg className="w-4 h-4 text-white/20 group-hover:text-[#D4AF37] group-hover:translate-x-0.5 group-hover:-translate-y-0.5 transition-all duration-300" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={1.5}>
                  ...
                  <div className="relative z-10">
                    <div className="w-6 h-[1px] bg-[#D4AF37]/40 mb-3 group-hover:w-10 transition-all duration-500" />
                    <h3 className="text-[14px] font-semibold text-white/90 uppercase tracking-wider leading-tight group-hover:text-[#D4AF37] transition-colors duration-500">
                      {area.title}
                    </h3>
                  </div>
                </div>
