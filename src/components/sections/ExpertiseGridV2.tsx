          ...
          {expertiseAreas.map((area, index) => (
            ...
            >
              <Link href={area.href} className="group block">
                <div className="relative bg-white/95 backdrop-blur-sm rounded-2xl shadow-[0_4px_20px_rgba(0,0,0,0.08)] border border-white/80 px-5 py-8 text-center hover:shadow-[0_12px_40px_rgba(212,175,55,0.15)] hover:-translate-y-2 transition-all duration-500 overflow-hidden">

                  {/* Left gold accent - always visible */}
                  <div className="absolute left-0 top-0 bottom-0 w-1 bg-gradient-to-b from-[#D4AF37] via-[#D4AF37]/50 to-transparent" />

                  {/* Icon */}
                  <div className="mb-5 mx-auto">
                    <svg className="w-8 h-8 text-[#D4AF37] mx-auto group-hover:scale-110 transition-transform duration-500" fill="none" stroke="currentColor" viewBox="0 0 24 24" strokeWidth={1.2}>
                      <path strokeLinecap="round" strokeLinejoin="round" d={area.icon} />
                    </svg>
                  </div>

                  {/* Title */}
                  <h3 className="text-[14px] font-bold text-primary-950 uppercase tracking-wider leading-tight">
                    {area.title}
                  </h3>

                  {/* Arrow indicator */}
                  <div className="mt-4 flex justify-center">
                    <div className="w-8 h-8 rounded-full border border-gray-200 flex items-center justify-center group-hover:border-[#D4AF37] group-hover:bg-[#D4AF37] transition-all duration-500">
                      <svg className="w-3.5 h-3.5 text-gray-300 group-hover:text-white transition-colors duration-500" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
