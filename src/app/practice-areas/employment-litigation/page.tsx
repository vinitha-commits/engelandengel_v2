...
export default function EmploymentLitigationPage() {
  ...
  const services = ['Historical Lost Earnings', 'Projected Lost Earnings', 'Lost Benefits', 'Mitigation Analysis', 'Expert Witness Testimony'];

  const publications = ['Framework for the Calculation of Employment Damages'];
          ...
          <div className="container-custom relative z-10 w-full pb-20 pt-40">
            <motion.div style={{ y: springY1, opacity }} className="max-w-5xl">
              <motion.div initial={{ opacity: 0, y: 30 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.8, ease: 'easeOut' }}>
                ...
                <h1 className="text-6xl md:text-8xl lg:text-9xl font-bold tracking-tighter text-white leading-[0.9]">Employment<br /><span className="font-serif italic text-[#D4AF37] font-medium">Litigation</span></h1>
            ...
            <div className="flex-1 min-w-0">
              ...
              <section className="py-10 my-10 border-t border-b border-slate-200">
                <motion.div initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ duration: 0.6 }}>
                  <h2 className="text-2xl md:text-3xl font-bold text-primary-950 leading-tight mb-4">How Engel &amp; Engel Helps Employment Litigation Attorneys</h2>
              ...
              <section className="border-slate-200">
                <motion.div initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ duration: 0.6 }}>
                  ...
                  <p className="mb-10">Engel &amp; Engel has published research in connection with employment damages:</p>
                  <div className="space-y-3">
                    {publications.map((item, i) => (
                      <motion.a href="/publications/employment-litigation" key={i} initial={{ opacity: 0, x: -15 }} whileInView={{ opacity: 1, x: 0 }} viewport={{ once: true }} transition={{ duration: 0.4, delay: i * 0.04 }} className="group flex gap-5 p-5 rounded-xl bg-white border border-transparent hover:border-primary-950/20 hover:shadow-md transition-all duration-300 items-center">
        ...
        <section className="relative py-28 bg-[#0A1A3C] overflow-hidden">
          ...
          <div className="container-custom relative z-10">
            <motion.div initial={{ opacity: 0, y: 30 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ duration: 0.8 }} className="max-w-5xl mx-auto">
              <p className="text-base md:text-lg text-white/60 font-light mb-10 text-center">For additional information about{' '}<span className="text-white font-medium">Engel &amp; Engel&apos;s</span>{' '}<span className="font-serif italic text-[#D4AF37]">Employment Litigation</span>{' '}or a consultation, please contact:</p>
