...
export default function RaffleAdminPage() {
  ...
  const fetchEntries = async (token?: string) => {
    ...
    try {
      ...
      if (winnerData.success && winnerData.winner) {
        ...
        setIsConfirmed(!!winnerData.winner.confirmed)
      }
    } catch (err) {
  ...
  const confirmWinner = async () => {
    ...
    setError('')
    try {
      const res = await fetch('/api/raffle/winner/confirm', {
        method: 'POST',
        headers: { 'x-admin-key': authToken },
      })
      const data = await res.json()
      if (!res.ok) throw new Error(data.error || 'Failed to confirm winner')
      if (data.success) {
        setIsConfirmed(true)
      }
    } catch (err: any) {
      setError(err.message || 'Failed to confirm winner')
  ...
  const exportCSV = () => {
    const headers = ['Name', 'Title', 'Email', 'Phone', 'Company', 'Submitted']
    const rows = entries.map(e => [e.name, e.title || '', e.email, e.phone, e.company, new Date(e.timestamp).toLocaleString()])
  ...
  // Dashboard
  return (
    <div className="min-h-screen bg-primary-950 overflow-x-hidden max-w-[100vw]">
      ...
      <div className="flex">
        ...
        <div className="hidden lg:flex w-64 min-h-screen bg-[#0d1f42] flex-col border-r border-white/5 fixed left-0 top-0 bottom-0 z-30">
          ...
          <div className="p-5 space-y-4 flex-1">
            ...
            {winner && (
              <div className={`${isConfirmed ? 'bg-green-500/10 border-green-500/30' : 'bg-[#D4AF37]/10 border-[#D4AF37]/30'} border rounded-xl p-4 text-center mb-3`}>
                <p className={`text-[10px] font-bold tracking-widest uppercase mb-2 ${isConfirmed ? 'text-green-400' : 'text-[#D4AF37]'}`}>
                  {isConfirmed ? 'Confirmed Winner' : 'Selected Winner'}
                </p>
            ...
            {winner && !isConfirmed && (
              <button
                ...
                className="w-full py-3 mt-2 bg-green-500 text-white font-bold text-sm rounded-xl hover:bg-green-600 transition-all duration-300 disabled:opacity-50"
              >
        ...
        <div className="flex-1 lg:ml-64">
          ...
          <div className="bg-gray-50 min-h-screen w-full overflow-x-hidden">
            <div className="p-4 md:p-6 lg:p-8 pb-32 lg:pb-8">
                        ...
                        >
                          ...
                          >
                            {!isConfirmed && (
                              <>
                                <button
                                  onClick={() => { setShowReveal(false); pickWinner(); }}
                                  className="px-8 py-3 bg-[#D4AF37] text-primary-950 text-sm font-bold rounded-lg hover:bg-white transition-colors"
                                >
                                  Pick Again
                                </button>
                                <button
                                  onClick={() => { confirmWinner(); setShowReveal(false); }}
                                  disabled={isConfirming}
                                  className="px-8 py-3 bg-green-500 text-white text-sm font-bold rounded-lg hover:bg-green-600 transition-colors disabled:opacity-50"
                                >
                                  {isConfirming ? 'Confirming...' : 'Confirm Winner'}
                                </button>
                              </>
